...
def generate_demo_custom_launch(moveit_config, launch_package_path=None):
    """
...
def generate_launch_description():
    ...
    baud_rate = "115200"
    for arg in sys.argv:
        if arg.startswith("serial_port:="):
            serial_port = arg.split(":=", 1)[1]
        elif arg.startswith("baud_rate:="):
            baud_rate = arg.split(":=", 1)[1]

    moveit_config = (
        MoveItConfigsBuilder("dummy-ros2", package_name="dummy_moveit_config")
        .robot_description(
            file_path="config/dummy-ros2.urdf.xacro",
            mappings={
    ...
    return generate_demo_custom_launch(moveit_config)
